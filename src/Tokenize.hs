...
tokenize ('\t':xs) = tokenize xs
tokenize ('\"':xs) = tokenizeLiteralString (Unfinished "") xs
...
tokenizeLiteralString :: PartialToken -> String -> [Token]
tokenizeLiteralString (Unfinished s) ('\"':xs) = [quote s] ++ tokenize xs
tokenizeLiteralString (Unfinished s) (x:xs)    = tokenizeLiteralString (Unfinished (s ++ [x])) xs
